...
class baseMysql{
    ...
    constructor(conf){
        ...
        this.lastSql = '' // 最后一次执行的sql语句
        this.lastErr = '' // 最后一次执行出错的记录
    ...
    query(sql, callback){
        ...
        this.pool
            .getConnection((err, db) => {
                if(err){
                    this.log.error('mysql connect error, ' + err)
                    return callback(err)
                }
    ...
    find(conf, callback){
        ...
        if(!conf.table){
            this.log.error('find error, empty table')
            return callback(null)
        }
    ...
    findOne(conf, callback){
        ...
        this.find({
            ...
            callback(null, list && list[0] || null)
        })
    }
    ...
    count(conf, callback){
        ...
        this.find({
            ...
            callback(null, res && res[0]['total'] || 0)
        })
    }
    ...
    insert(conf, callback){
        ...
        if(!conf.table || (typeof conf.table !== 'string')){
            this.log.error('insert error, empty table')
            return callback(null)
        }
    ...
    update(conf, callback){
        ...
        if(!conf.table || (typeof conf.table !== 'string')){
            this.log.error('update error, empty table')
            return callback(e)
        }
    ...
    remove(conf, callback){
        ...
        if(!conf.table || (typeof conf.table !== 'string')){
            this.log.error('remove error, empty table')
            return callback(e)
        }
    ...
    parseWhere(arr){
        ...
        if(typeof arr === 'string'){
            return ' WHERE ' + arr
        }else if(typeof arr === 'object' && Object.empty(arr)){
            ...
            for(let it of arr){
                ...
                if(/[^\d\.]/.test(it.val) && isNaN(it.val - 0) && !['IN', 'BETWEEN'].includes(it.op))
                    it.val = `"${it.val}"`
