...
class baseRethink {
    ...
    constructor(conf){
        ...
        this.promise = rt.connect(_this.conf)
    ...
    find(conf, callback){
        this.promise.then(conn => {
            ...
            if(conf.hasOwnProperty('select') && conf.select.length > 0)
                query.withFields.apply(query, conf.select)
    ...
    update(conf, callback){
        ...
        this.promise.then(conn => {
            ...
            let table = rt.table(conf.table).filter(conf.where)
            ...
            if(conf.hasOwnProperty('$unset'))
                ...
                table.update(conf.$set)
            else
                table.update(conf.update)

            table.run(conn, opts, callback)
        }).error(e => {
