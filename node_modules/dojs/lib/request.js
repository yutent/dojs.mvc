    ...
    post(key, xss){
        ...
        if(postType === 1){
            ...
            return fn => {
                ...
                this.req.on('end', err => {
                    para = libs.QS.parse(postData)

                    try{
                        para = JSON.parse(para)
                    }catch(e){}

                    if(!/urlencoded/.test(this.headers('content-type'))){
                        para = Object.keys(para)[0]
                    }else{
                        for(let key in para){
                            if(key.slice(-2) === '[]'){

                                let tmpkey = key.slice(0, key.indexOf('['))
                                para[tmpkey] = (para[key])
                                delete para[key]
